...
'response.write upload.Version&"<br><br>"  ''显示上传类的版本
	...
	if right(formPath,1)<>"/" then formPath=formPath&"/" 
			...
			if oFile.FileSize>=50000 then 
			 %>
			 <script language=javascript>
                     history.back()
                     alert("上传文件不能超过不能超过50K！您输入的文件大小为{ <%=imageSize%> }已超过了限制")
             </script>
             <%
            response.end
			end if		
			...
			'判断是否上传过图片
			if l_picIsNull="True" then 'w_pic 字段为空更新w_pic字段值
...
<head>
<title>modipic</title>
<meta http-equiv="refresh" content="0;url=ProBigClass_List.asp?pageno=<%= pageno %>&l_id=<%= l_id %>">
