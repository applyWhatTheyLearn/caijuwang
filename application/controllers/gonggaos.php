...
class Gonggaos_Controller extends Controller {
    ...
    function info($view){
        $page = Input::get('page');
        $id = Input::get('w_id');
        $gonggao = DB::table('w_notice')->where('w_id','=',$id)->get();
        $view = View::make($view)
          ->with('gonggao',$gonggao[0])
            ->with('page',$page);
        return $view;
    }
    ...
    public function get_detail(){
        return Gonggaos_Controller::info('admin.gonggao.gonggao_detail');
    }

    public function get_modi(){
        return Gonggaos_Controller::info('admin.gonggao.gonggao_modi');
